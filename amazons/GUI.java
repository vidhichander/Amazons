...
class GUI extends TopLevel implements View, Reporter {
    ...
    GUI(String title) {
        ...
        addMenuButton("Game->New", this::new1);
        addMenuButton("Settings->Seed", this::newSeed);
        addMenuButton("Manual->Color", this::makeManual);
        addMenuButton("Auto->Color", this::makeAuto);
    ...
    private void new1(String dummy) {
    ...
    private void newSeed(String dummy) {
        ...
        if (response != null) {
            ...
            if (mat.matches()) {
                _pendingCommands.offer(String.format("seed %s",
                        mat.group(1)));
            } else {
    ...
    private void makeManual(String dummy) {
        String response =
                getTextInput("Enter color to make manual.",
                        "Make Manual", "plain", "");
        if (response != null) {
            if (response.equalsIgnoreCase("white")
                    || response.equalsIgnoreCase("black")) {
                _pendingCommands.offer(String.format("manual %s", response));
            } else {
                showMessage("Enter a color please.", "Error", "error");
            }
        }
    }

    /** Response to "Auto" button click. */
    private void makeAuto(String dummy) {
        String response =
                getTextInput("Enter color to make auto.",
                        "Make Auto",  "plain", "");
        if (response != null) {
            if (response.equalsIgnoreCase("white")
                    || response.equalsIgnoreCase("black")) {
                _pendingCommands.offer(String.format("auto %s", response));
            } else {
                showMessage("Enter a color please.", "Error", "error");
            }
        }
