...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("([a-j](?:[1-9]|10))\\s+([a-j](?:[1-9]|10))\\s+"
    ...
    /** Make AUTO the automated played. */
    private void doAuto(Matcher auto) {
        String player = auto.group(1);
    ...
    /** Make AUTO the manual played. */
    private void doManual(Matcher auto) {
        String player = auto.group(1);
        if (player.compareTo("white") == 0) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else {
    ...
    private void doMove(Matcher move) {
        String from = move.group(1);
        String to = move.group(2);
        String spear = move.group(3);
        try {
            if (Move.mv(from + " " + to + " " + spear) != null) {
                _board.makeMove(Move.mv(from + " " + to + " " + spear));
                if (_board.turn() == BLACK) {
                    Set<Move> moves = new HashSet<>();
                    Iterator<Move> legalMoves = _board.legalMoves(Piece.BLACK);
                ...
                if (_board.turn() == WHITE) {
                    Set<Move> moves = new HashSet<>();
                    Iterator<Move> legalMoves = _board.legalMoves(Piece.WHITE);
                    while (legalMoves.hasNext()) {
                        Move m = legalMoves.next();
                        if (m != null) {
                            moves.add(m);
